...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):#每个句子，句首尾加{和}表示start token和end token，每个词为一个list(int)，每句list(list(int))
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded=pad_sents_char(sents_indices,self.char_pad)
        sents_var=torch.tensor(sents_padded,device=device).transpose(0,1)
        return sents_var
