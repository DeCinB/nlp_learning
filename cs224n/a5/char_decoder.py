...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        s_t,dec_hidden=self.forward(input)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        current_char_id=torch.tensor([[self.target_vocab.start_of_word]*batch_size],device=device)
        #current_char(1,batch_size)当前都为start_token
        decodeWord=['']*batch_size
        #decodeWord (batch_size)

        decode_finished=[False]*batch_size#标记

        for _ in range(max_length):
            scores,dec_hidden=self.forward(current_char_id,dec_hidden)
            #score(1,batchsize,vocab_size)
            current_char_id=scores.argmax(dim=2)
            #current_char_id(1,batchsize)
            for i,pred_char_id in enumerate(current_char_id.detach().squeeze(0)):
                if pred_char_id is not self.target_vocab.end_of_word and not decode_finished[i]:
                    pred_char=self.target_vocab.id2char[pred_char_id.item()]
                    decodeWord[i]+=pred_char
                else:
                    decode_finished[i]=True
