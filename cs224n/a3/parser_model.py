...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden_weight=nn.init.xavier_uniform_(torch.empty(self.embed_size*self.n_features,self.hidden_size))
        self.embed_to_hidden_weight=nn.Parameter(embed_to_hidden_weight)
        embed_to_hidden_bias=nn.init.uniform_(torch.empty(self.hidden_size))
        self.embed_to_hidden_bias=nn.Parameter(embed_to_hidden_bias)
        hidden_to_logits_weight=nn.init.xavier_uniform_(torch.empty(self.hidden_size,self.n_classes))
        self.hidden_to_logits_weight=nn.Parameter(hidden_to_logits_weight)
        hidden_to_logits_bias=nn.init.uniform_(torch.empty(self.n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        for sample in w:
            embed=torch.index_select(self.embeddings,0,sample).view(1,-1)
            try:
                x=torch.cat((x,embed),0)
            except NameError:
                x=embed
    ...
    def forward(self, w):
        ...
        embed_feature=self.embedding_lookup(w)
        h=F.relu(torch.matmul(embed_feature,self.embed_to_hidden_weight)+self.embed_to_hidden_bias)
        h=self.dropout(h)
