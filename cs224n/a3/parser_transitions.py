...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack=['ROOT']
        self.buffer=sentence.copy()#不能更改原句
    ...
    def parse_step(self, transition):
        ...
        if transition=='S':
            ...
            head,dp=self.stack.pop(),self.stack.pop()
            self.dependencies.append((head,dp))
            self.stack.append(head)
        elif transition=="RA":
            dp,head=self.stack.pop(),self.stack[-1]
            self.dependencies.append((head,dp))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses=[PartialParse(sen) for sen in sentences]#为每个句子生成partial_parse对象
    unfinished_parses=partial_parses[:]#未完成依赖性分析的parse
    ...
    while unfinished_parses:
        if batch_size<=len(unfinished_parses):
            ...
            unfinished_parses=None
        parse_index=[i for i in range(len(minibatch_parses))]#原始索引，便于与batch_dependencies对应
        batch_dependencies=[[]]*len(minibatch_parses)

        while minibatch_parses:
            transitions=model.predict(minibatch_parses)
            for i in range(len(minibatch_parses)-1,-1,-1):#逆序可使得删除便捷,顺序删除会改变该位之后元素与原先索引的对应关系
                minibatch_parses[i].parse_step(transitions[i])
                if len(minibatch_parses[i].buffer)==0 and len(minibatch_parses[i].stack)==1:#buffer为空，stack只有一个元素，完成分析
                    batch_dependencies[parse_index[i]]=minibatch_parses[i].dependencies
                    del minibatch_parses[i]#删除
                    del parse_index[i]
