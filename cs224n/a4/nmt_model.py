...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #压紧序列
        packed_seq=pack_padded_sequence(X,source_lengths,enforce_sorted=True)
        enc_hiddens,(last_hidden,last_cell)=self.encoder(packed_seq)
        #把压紧的序列填充回来
        enc_hiddens=pad_packed_sequence(sequence=enc_hiddens)[0].permute(1,0,2)#重排维度
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y,1,dim=0):
            ...
            Y_t_bar=torch.cat((Y_t,o_prev),dim=1)
            dec_state,o_t,e_t=self.step(Y_t_bar,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #dec_hidden(b, h),enc_hiddens_proj(b, src_len, h),e_t(b, src_len)
        #enc_hiddens unsqueeze后为(b,h,1),bmm后得到(b,src_len,1),再squeeze得到(b,src_len)
        ...
        alpha_t=F.softmax(e_t)#alpha_t(b,str_len),enc_hiddens(b,str_len,2h),a_t(b,2h)
        a_t=torch.squeeze(torch.bmm(enc_hiddens.permute(0,2,1),torch.unsqueeze(alpha_t,dim=2)),dim=2)
